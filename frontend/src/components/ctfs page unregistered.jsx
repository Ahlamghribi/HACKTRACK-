...
const SavedEventsSection = () => {
  ...
  useEffect(() => {
    const fetchCTFs = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/CTFs');
  ...
  useEffect(() => {
    const now = new Date();
    console.log(events.CTFs);
    const CTFs = Array.isArray(events.CTFs) ? events.CTFs : [];
    const filtered = CTFs.filter((event) => {
  ...
  return (
    <div className="p-8 bg-[#1A232E] min-h-screen">
       <h1 className="text-white font-['Necosmic'] text-3xl mt-12 mb-12 text-center">
        <span className="text-white">CTFS</span> 
      </h1>
          ...
          <tbody>
            ...
            {filteredEvents.map((event, index) => (
              ...
              >
                ...
                <td className="p-4">
                  <Link to="/ctfdiscover" state={event} className="px-4 py-2 bg-purple-500 rounded-full hover:bg-purple-400 font-['New'] transition-colors">
