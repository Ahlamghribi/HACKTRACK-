...
const SavedEventsSection = () => {
  ...
  useEffect(() => {
    const FetchDatathons = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/datathons');
        setEvents(response.data);
      } catch (err) {
        console.error('Error fetching Datathons:', err);
      }
    };
  ...
  useEffect(() => {
    const now = new Date();
    console.log(events.datathons);
    const datathons = Array.isArray(events.datathons) ? events.datathons : [];
    const filtered = datathons.filter((event) => {
          ...
          <tbody>
            ...
            {filteredEvents.map((event, index) => (
              ...
              >
                ...
                <td className="p-4">
                  <Link to="/datathondiscover" state={event} className="px-4 py-2 bg-purple-500 rounded-full hover:bg-purple-400 font-['New'] transition-colors">
    ...
    <div className="min-h-screen bg-[#1A232E] relative">
      <nav className="flex justify-between items-center px-30 py-16 bg-transparent ml-12">
        <div className="flex space-x-32 items-center">
          ...
          <Link to="/signup" className="bg-purple-500 px-4 py-2 rounded-lg text-white font-['New'] text-[22px] hover:bg-purple-400">
                                Register Now
                              </Link>
